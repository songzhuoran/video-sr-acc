...
modelName = '/home/songzhuoran/video/video-sr-acc/train_info/model_fc4'
    ...
    def __getitem__(self,index):
        ...
        for i in range(0,len(self.classname_list)):
            ...
            if index<self.total[i]:
                ...
                # input_frequency = test[1].reshape((-1,))
                # label_frequency = test[2].reshape((-1,)) # need to modify!!!!!!
...
if __name__ == '__main__':
    ...
    RESULT_DIR = "/home/songzhuoran/video/video-sr-acc/bframe_sr_only_warp/"
    ...
    net=torch.load(NN_INFO_DIR + "model_fc4").to(device)
    ...
    for data in train_loader:
        ...
        # get 8*8 hight-resolution MV
        # cur_idx, ref_idx, block_w, block_h, curx, cury, refx, refy = MV.data.numpy()[0,:]
        if cur_idx in bflist[class_num]: #only reconstruct B frame
            #init a B frame array
            """
            if prev_idx == -1:
                B_img = np.zeros((sr_frame_h,sr_frame_w, 3), dtype="uint8")
                # prev_idx = cur_idx
            elif ((prev_idx != -1) and (cur_idx != prev_idx)) or ():
                print("write back a B frame, index = ", prev_idx)
                B_img=cv2.cvtColor(B_img, cv2.COLOR_RGB2BGR)
                cv2.imwrite(RESULT_DIR+classname+"/%08d.png" % prev_idx, B_img)
                print("init B frame!")
                B_img = np.zeros((sr_frame_h,sr_frame_w, 3), dtype="uint8")
                # prev_idx = cur_idx
            """
            ...
            # fre_res_block = output.cpu().data.numpy()
            # fre_res_block = fre_res_block.reshape((8,8,3))
            fre_res_block = label_frequency # need to modify!!!!
            fre_res_block = labelPostprocess(fre_res_block.cpu().data.numpy())
            fre_res_block = fre_res_block.reshape((8,8,3))
            # print(fre_res_block)
            ...
            # tmp_fre_res_block = fre_res_block #need to modify!!!!!
            ...
            # tmp_tmp_fre_res_block = fre_res_block #need to modify!!!!!
