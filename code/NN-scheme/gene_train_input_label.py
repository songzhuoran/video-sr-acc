...
# overall_info = np.empty([1,72], dtype = float) # an array to store all info, including MV and frequency
# classname = 'calendar'
# classname = 'city'
# classname = 'foliage'
# classname = 'walk'
...
def bframe_gen_kernel(fcnt):
    ...
    global overall_info
    # print(fcnt)
    # print("overall_info size: ",len(overall_info))
    # f = open(TRAIN_DIR + classname + '.csv','a',newline='')
    # writer = csv.writer(f)
    # tmp_img=cv2.cvtColor(cv2.imread("/home/songzhuoran/video/video-sr-acc/bframe_sr_only_warp/calendar/00000002.png"), cv2.COLOR_BGR2RGB) ## need to modify!!!!
    for row in MV_data:
        if int(float(row[0])) == fcnt:
        # if int(float(row[0])) == fcnt and fcnt == 2: ## need to modify!!!!
            # if (fcnt != 2) :
            ...
            for px in range(block_w):
                for py in range(block_h):
                    if (curx+px in range(frame_w)) and (cury+py in range(0, frame_h)):
                        ...
                        # print(b_block_gt.shape) # (4,4,3)
                        # print(b_block_up.shape) # (4,4,3)
                        if (refx + px in range(0, frame_w)) and (refy+py in range(0, frame_h)):
                            ...
                            # input_residual[(py*4):(py*4+4),(px*4):(px*4+4),:] = b_block_up ##need to modify!!!
                            # label_residual[(py*4):(py*4+4),(px*4):(px*4+4),:] = b_block_gt
                        else:
            ...
            # tmp_input_residual = input_residual # need to modify!!!!!!!
            # tmp_label_residual = label_residual
            ...
            for px in range(int(4*block_w/8)):
                for py in range(int(4*block_h/8)):
                    ...
                    # tmp_tmp_input_residual = tmp_input_residual[py*8:py*8+8,px*8:px*8+8,:]
                    # tmp_tmp_label_residual = tmp_label_residual[py*8:py*8+8,px*8:px*8+8,:] ## need to modify!!!!!
                    ...
                    # print(tmp_curx,tmp_cury,tmp_refx,tmp_refy)
    ...
    # cv2.imwrite("/home/songzhuoran/video/video-sr-acc/bframe_sr_only_warp/calendar/00000002.png",cv2.cvtColor(tmp_img, cv2.COLOR_RGB2BGR)) ## need to modify!!!!
...
for i in classname_list:
    ...
    #begin function
    she = shelve.open(TRAIN_DIR+"train.bat")
    # she = shelve.open(TRAIN_DIR+"test.bat") ##need to modify!!!
    fetch_MV_data()
    dep_tree_gen()
