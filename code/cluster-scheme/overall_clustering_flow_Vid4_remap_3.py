## 针对Vid4数据集；使用了二次运动估计的clustering算法，三层聚类
import warnings
...
def Reconstruct_centroid_res_delta_func_3(ratio_delta,ratio):
    for i in classname_list:
        ...
        overall_delta_label_3_list = cluster[classname]
        cluster.close()
        cluster_res = shelve.open(TRAIN_DIR+"cluster_delta_res_3_r"+str(ratio)+".bat")
        overall_centroid_delta_res_3_list = cluster_res[classname]
        cluster_res.close()
        ...
        for label_cnt in range(len(overall_cluster_label)):
            ...
            cluster_delta_label_3 = overall_delta_label_3_list[label_cnt] # delta label
            centroid_delta_res_3 = overall_centroid_delta_res_3_list[label_cnt] # delta residual
            for label_idx, label_data in enumerate(cluster_label):
                ...
                delta_label_3_data = cluster_delta_label_3[label_idx]
                if ratio_delta != 0:
                    delta_res = centroid_delta_res[delta_label_data]
                    delta_res_3 = centroid_delta_res_3[delta_label_3_data]
                else:
                    delta_res = centroid_delta_res[int(delta_label_data)]
                    delta_res_3 = centroid_delta_res_3[int(delta_label_3_data)]
                

                tmp_res = single_res + delta_res + delta_res_3
...
Cluster_delta_func(ratio_2,ratio)
# ### third floor clustering
Cluster_delta_func_3(ratio_3,ratio)


# ### reconstruction
Reconstruct_centroid_res_delta_func_3(ratio_3,ratio)
