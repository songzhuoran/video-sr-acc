...
SLICES_OUT_DIR = '/home/songzhuoran/video/video-sr-acc/train_info/REDS_Cluster_remap/IP_mix/'
...
def remapSliceKernel(fidx, mvs_data, bflist, dic):
    ...
    for i, row in enumerate(mvs_data):
        ...
        for ref in refs:  # 在所有相关帧里重新搜索最匹配的块
            if ref in bflist:
                # refFrame = cv2.imread(GT_PICS_DIR+classname+ "/frame_%04d.png" % (ref+1)) # add by songzhuoran,sintel dataset
