...
SLICES_OUT_DIR = '/home/songzhuoran/video/video-sr-acc/train_info/REDS_Cluster_remap/mix/'
...
def remapSliceKernel(fidx, mvs_data, bflist, slice_dict):
    '''
    ...
    for i, row in enumerate(mvs_data):
        ...
        # curFrame = cv2.imread(
        #     GT_PICS_DIR + classname + "/frame_%04d.png" % (curId+1))
        curFrame = cv2.imread(
            GT_PICS_DIR + classname + "/%08d.png" % curId)
        ...
        for ref in refs:  # 在所有相关帧里重新搜索最匹配的块
            if ref in bflist:
                # refFrame = cv2.imread(GT_PICS_DIR+classname+"/frame_%04d.png" % (ref+1))
                ...
                # refFrame = cv2.imread(SR_PICS_DIR + classname + "/frame_%04d.png" % (ref+1))
...
classnameList = ['006','007','008','009','010','011','012','013','014','015','016','017','018','019','020','021','022','023','024','025','026','027','028','029']
