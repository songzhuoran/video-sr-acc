#### 针对Vid4数据集；I/P帧为ground truth，用于与传统encoder比较
import warnings
...
B_DIR="/home/songzhuoran/video/video-sr-acc/Vid4/Our_result/bframe_sr_GT/" # SR result
            ...
            if cur_idx > init_p_idx:
                ...
                if init_p_cnt < len(pflist):
                    ...
                    if int(res_arr.shape[0]/ratio) != 0:
                        ...
                        # ms = KMeans(n_clusters=int(res_arr.shape[0]/ratio),verbose=1,algorithm='full',n_init = 10)
                        ms = KMeans(n_clusters=int(res_arr.shape[0]/ratio),verbose = 1,n_init = 10)
...
def Cluster_clustered_res_func(ratio_clustered,ratio):
    ...
    for i in classname_list:
        ...
        for label_cnt in range(len(overall_cluster_label)):
            ...
            res_arr = np.array(res_list)
            # ms = KMeans(n_clusters=int(res_arr.shape[0]/ratio_clustered),verbose=1,algorithm='full',n_init = 10)
            ms = KMeans(n_clusters=int(res_arr.shape[0]/ratio_clustered),verbose = 1,n_init = 10)
...
def Cluster_delta_func(ratio_delta,ratio):
    for i in classname_list:
        ...
        for label_cnt in range(len(overall_delta_res_list)):
            ...
            if ratio_delta !=0:
                # ms = KMeans(n_clusters=int(delta_res_arr.shape[0]/ratio_delta),verbose=1,algorithm='full',n_init = 10)
                ms = KMeans(n_clusters=int(delta_res_arr.shape[0]/ratio_delta),verbose = 1,n_init = 10)
