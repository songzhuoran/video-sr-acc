### code for reconstructing 8*8 residual blocks using clustering algorithm
import warnings
...
def reconstruct_delta_cluster_func(ratio_delta,ratio,interval):
    for i in classname_list:
        ...
        for label_cnt in range(len(overall_cluster_label)):
            ...
            cluster_label = overall_cluster_label[label_cnt]
            clustered_res = overall_clusered_res[label_cnt]
            cluster_delta_label = overall_clustered_delta_label_list[label_cnt] # delta label
            clustered_delta_res = overall_clustered_delta_res_list[label_cnt] # delta residual
            # print(len(cluster_label))
            # print(len(clustered_res))
            # print(len(cluster_delta_label))
            # print(len(clustered_delta_res))
            for label_idx, label_data in enumerate(cluster_label):
                # locate MV
                MV_idx = label_idx + begin_cnt
                tmp_MV = block_MV_list[MV_idx]
