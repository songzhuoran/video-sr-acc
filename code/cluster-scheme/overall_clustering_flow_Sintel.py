...
#directory
TRAIN_DIR = "/home/songzhuoran/video/video-sr-acc/train_info/Sintel_Cluster/"
IDX_DIR = "/home/songzhuoran/video/video-sr-acc/Sintel/Info_BIx4/idx/" # idx directory
B_DIR="/home/songzhuoran/video/video-sr-acc/Sintel/Our_result/bframe_sr_reconstruction/" # SR result
PICS_DIR = "/home/songzhuoran/video/video-sr-acc/Sintel/BIx4/" # GT_LR_pic
residual_info = [] # a list to store all info, including MV and frequency
classname_list = ['ambush_1', 'market_4', 'temple_1', 'mountain_2', 'bamboo_3', 'wall', 'market_1', 'PERTURBED_market_3', 'cave_3', 'PERTURBED_shaman_1', 'ambush_3', 'tiger']
# classname_list = ['ambush_1']
classname = 'ambush_1'
MV_list = []
res_list = []
...
def Reconstruct_centroid_res_delta_func(ratio_delta,ratio):
    for i in classname_list:
        ...
        for i in range(frame_num):
            tmp_img = cv2.imread(B_DIR + classname + "/frame_%04d.png" % (i+1)) # read SR result
        ...
        for label_cnt in range(len(overall_cluster_label)):
            ...
            for label_idx, label_data in enumerate(cluster_label):
                ...
                if tmp_cur_idx == -1:
                    ...
                    B_img = cv2.imread(B_DIR + classname + "/frame_%04d.png" % (cur_idx+1), -1)
                    ...
                    cv2.imwrite(B_DIR + classname + "/frame_%04d.png" % (tmp_cur_idx+1), B_img)
                    print("init a B frame, the idx is %d: ", cur_idx)
                    B_img = cv2.imread(B_DIR + classname + "/frame_%04d.png" % (cur_idx+1), -1)
                    B_img = cv2.cvtColor(B_img, cv2.COLOR_BGR2RGB).astype("float")
                tmp_cur_idx = cur_idx
